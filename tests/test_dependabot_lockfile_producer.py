...
artifact of `requirements.in`, and `pyproject.toml`'s nousergon-lib pin is
held in lockstep with the tag `requirements.in` declares. Dependabot's pip
ecosystem edits the two derived files and knows nothing about the source
file, so every pip PR it opened was red on `lockfile-reproducible` and on
`test` before a dependency was graded (measured on crucible-executor#511,
2026-08-28) — and Dependabot PRs are a STANDING auto-merge exception, so the
fleet believed this repo had automated dependency maintenance while it had
none.
...
which is the same dead-exception failure in a new costume.
"""
...
VERIFIER = ROOT / "scripts" / "check_lock_reproducible.sh"
...
def test_the_flag_parity_check_fails_on_a_seeded_drift(tmp_path):
    ...
    (fake / "scripts").mkdir()
    shutil.copy(VERIFIER, fake / "scripts" / VERIFIER.name)
    ...
    ver = _capture_uv_argv(tmp_path / "dv", fake / "scripts" / VERIFIER.name, [])
    assert _normalise(prod) != _normalise(ver), (
