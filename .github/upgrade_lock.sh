...
# Dependabot edits requirements.txt and pyproject.toml directly and has no
# knowledge of requirements.in, so every pip PR it opened here was red on
# `lockfile-reproducible` ("requirements.txt is not reproducible from
# requirements.in") AND on `test` (pyproject's nousergon-lib pin bumped past
# the tag requirements.in declares) before a single dependency was graded.
# A bot that cannot open a green PR is not dependency maintenance; it is a
# queue of red PRs that trains the reader to ignore the ones that matter.
#
